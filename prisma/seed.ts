...
const generatePhone = () =>
  `+92${Math.floor(3000000000 + Math.random() * 999999999)}`;
const generateEmergencyPhone = () =>
  `+92${Math.floor(3000000000 + Math.random() * 999999999)}`;
const generateCarNo = () =>
  `${String.fromCharCode(65 + Math.floor(Math.random() * 26))}${String.fromCharCode(65 + Math.floor(Math.random() * 26))}-${Math.floor(100 + Math.random() * 900)}`;
const generateEmail = (prefix: string, index: number) =>
  `${prefix}${index + 1}@example.com`;
...
const lahoreLocations = [
  ...
  { name: "Raiwind Road", lat: 31.389, long: 74.1989 },
...
async function main() {
  ...
  try {
    ...
    // Delete non-admin users (drivers and passengers first, then their users)
    await prisma.user.deleteMany({
      where: {
        AND: [
          { email: { not: "admin@falcon.com" } },
          { is_super_admin: false },
        ],
      },
    });
  } catch (e) {
    console.log("⚠️ Some tables don't exist yet (first run), skipping cleanup");
  }
  ...
  const passengerRole = await prisma.role.findUnique({
    where: { name: "passenger" },
  });
  if (!passengerRole) throw new Error("Passenger role not found");
  ...
  console.log("👨‍✈️ Creating 100 drivers with users...");
  const drivers = [];
  for (let i = 0; i < 100; i++) {
    const city = lahoreLocations[i % lahoreLocations.length];
    const driverName = driverNames[i % driverNames.length];
    const driverEmail = `driver${i + 1}@example.com`;
  ...
  console.log("🧑‍💼 Creating 100 passengers with users...");
  const passengers = [];
  for (let i = 0; i < 100; i++) {
    const city = lahoreLocations[i % lahoreLocations.length];
    const passengerName = passengerNames[i % passengerNames.length];
    const passengerEmail = `passenger${i + 1}@example.com`;
    ...
    const passenger = await prisma.passenger.create({
      data: {
        ...
        pick_up_time: `${7 + Math.floor(Math.random() * 3)}:${Math.floor(
          Math.random() * 60,
        )
          .toString()
          .padStart(2, "0")} AM`,
        drop_off_time: `${5 + Math.floor(Math.random() * 4)}:${Math.floor(
          Math.random() * 60,
        )
          .toString()
          .padStart(2, "0")} PM`,
        office_pick_up_time: `${(5 + Math.floor(Math.random() * 3)).toString().padStart(2, "0")}:${Math.floor(
          Math.random() * 60,
        )
          .toString()
          .padStart(2, "0")} PM`,
  ...
  console.log("🛣️ Creating 100 routes...");
  const routes = [];
  for (let i = 0; i < 100; i++) {
    const city = lahoreLocations[i % lahoreLocations.length];
  ...
  for (let i = 0; i < 100; i++) {
    ...
    await prisma.routeLeg.create({
      data: {
        ...
        toll_amount:
          Math.random() > 0.7 ? 50 + Math.floor(Math.random() * 200) : null,
      },
    });
  }

  console.log("⚙️ Creating driver configuration...");
  await prisma.driverConfiguration.create({
    data: {
  ...
  console.log(`   - Driver-Car Assignments: 100`);
  console.log(`   - Passengers: ${passengers.length}`);
  console.log(`   - Routes: ${routes.length}`);
  console.log(`   - Route Batches: ${batches.length}`);
  console.log(`   - Route Segments: 100`);
  console.log(`   - Route Legs: 100`);
}
