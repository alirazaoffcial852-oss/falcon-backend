...
export function buildPassengerReportFromRouteLegs(
	...
	const passengers = [...byPassenger.values()].map((p) => {
		const dropOfficeTime = resolveDropOfficePickUpTime(
			waypointMode,
		...
		return {
			...
			phone_no: p.phone_no,
			pickup: {
				phase_passenger_id: null as unknown as number,
				status: null as unknown as PhasePassengerStatus,
				picked_up,
			...
			drop: {
				...
				status: null,
				dropped_off,
				actual_pickup_time: dropOfficeTime,
